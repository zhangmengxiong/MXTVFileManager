 ...
 */
class FileTypeScanTask(private val postExecute: AsyncPostExecute<ArrayList<FileBean>>?, private val rootDir: String, private val fileTypeBean: FileTypeBean) : AsyncTask<String, String, ArrayList<FileBean>>() {
    private val fileBeanArrayList = ArrayList<FileBean>()
    ...
    override fun doInBackground(vararg strings: String): ArrayList<FileBean> {
        fileBeanArrayList.clear()
        FileScanUtil.getChildFile(rootDir, fileTypeBean, object : FileScanUtil.FindCall {
            override fun findOneFile(path: String) {
                fileBeanArrayList.add(FileBean(File(path)))
            }
        })
